...
import { GCal, isGCalEnabled, fetchExternalGCalEvents } from '../lib/googleCalendar'
...
export default function Calendario() {
  ...
  const [gcalSyncing,    setGcalSyncing]    = useState(false) // kept for UI compat

  useEffect(() => {
    isGCalEnabled(supabase).then(setGcalConnected)
  }, [])
  ...
  const fetchGcalEvents = useCallback(async (dateMin, dateMax) => {
    try {
      const items = await fetchExternalGCalEvents(dateMin, dateMax, supabase)
      setGcalEvents(items.map(item => ({
        id:          `gcal_${item.id}`,
        tipo:        'gcal',
        titulo:      item.title,
        fecha:       item.fecha,
    ...
    <div className="flex flex-col h-full bg-[#fafafa] overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-100 px-5 py-3 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          {gcalConnected ? (
            <span className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-xl border border-blue-100 bg-blue-50 text-blue-600">
