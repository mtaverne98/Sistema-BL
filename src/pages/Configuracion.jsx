...
import {
  getDriveAuthUrl, checkDriveConnection, disconnectDrive,
} from '../lib/driveApi'
...
function GoogleDriveCard() {
  ...
  useEffect(() => {
    checkDriveConnection(supabase).then(ok => { setConnected(ok); setLoading(false) })
  }, [])
  ...
  useEffect(() => {
    ...
    if (drive === 'connected') {
      checkDriveConnection(supabase).then(ok => setConnected(ok))
      window.history.replaceState({}, '', '/configuracion')
    } else if (drive === 'error') {
      const msg = params.get('msg') || 'Error al conectar Google Drive'
      setStatus('error')
      setStatusMsg(decodeURIComponent(msg))
      window.history.replaceState({}, '', '/configuracion')
    }
  }, [location.search])

  async function handleDisconnect() {
    await disconnectDrive(supabase)
    setConnected(false)
    setStatus(null)
    setStatusMsg('')
  }
  ...
  return (
    <div className="p-5">
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-white border border-gray-100 shadow-sm flex items-center justify-center flex-shrink-0">
            <HardDrive size={18} className="text-[#2570BA]" />
          </div>
          <div>
            <p className="text-sm font-bold text-[#1a2e4a]">Google Drive</p>
            <p className="text-xs text-gray-400">Subida de documentos a la unidad compartida del estudio</p>
          </div>
        </div>
        {loading ? (
          <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full bg-gray-50 text-gray-300 border border-gray-100 flex-shrink-0">
      ...
      {/* Error */}
      {status === 'error' && (
        <div className="flex items-center gap-2 text-xs font-medium px-3.5 py-2.5 rounded-xl bg-red-50 text-red-600 border border-red-100 mb-4">
        ...
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 flex gap-3">
            ...
            <div className="text-xs text-blue-700 leading-relaxed space-y-1">
              ...
              <ul className="space-y-0.5 text-blue-600">
                <li>· Sube documentos desde la ficha de cada causa directamente a Drive</li>
                <li>· Los archivos quedan en la unidad compartida del estudio, organizados por cliente y causa</li>
                <li>· Solo accede a archivos que la app crea — no a todo tu Drive</li>
              </ul>
            </div>
          </div>
          <button
            onClick={() => { window.location.href = getDriveAuthUrl() }}
            className="w-full flex items-center justify-center gap-2.5 bg-[#2570BA] text-white text-sm font-semibold px-5 py-3 rounded-xl hover:bg-[#2570BA]/90 transition-colors shadow-sm"
          >
            <HardDrive size={15} />
      ...
      {connected && (
        <div className="space-y-4">
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl px-4 py-3 flex gap-3">
            <CheckCircle size={14} className="text-emerald-500 flex-shrink-0 mt-0.5" />
            <p className="text-xs text-emerald-700">Drive conectado. Puedes subir documentos desde la pestaña Documentos en cada causa.</p>
          </div>
          <div className="pt-2 border-t border-gray-50">
            <button
              onClick={handleDisconnect}
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#fafafa]">
      <div className="max-w-2xl mx-auto px-6 py-8">
        ...
        <Section
          title="Integraciones"
          description="Conecta Google para sincronizar audiencias y subir documentos"
        >
          <GoogleCalendarCard />
